...
from pything import Union, List, Tuple, Dict
...
def extract_function_calls(code) -> List[Tuple[str, Union[str, None], List[str]]]:
    logger.info(f"Extracting function calls from code: {code}")
    ...
    class FunctionCallVisitor(ast.NodeVisitor):
        ...
        def visit_FunctionDef(self, node):
            logger.info(f"Visiting function: {node.name}")
            """
...
def construct_function_map(code_path, save_path):
    logger.info(f"Constructing function map from {code_path}")
    """
